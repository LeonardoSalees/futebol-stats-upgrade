...
export async function DELETE(
  ...
  try {
    ...
    if (existingRound.finished) {
      return new Response(
        JSON.stringify({ error: "Não é possível excluir uma rodada finalizada" }),
        { status: 403 }
      );
    }
    ...
    console.error("Erro ao excluir a rodada:", error);
    return new Response(
      JSON.stringify({ error: "Erro ao excluir a rodada" }),
      { 
        status: 500,
        headers: {
          'Content-Type': 'application/json'
